...
/**
 * `useGeolocation` returns a `GeoLocationSensorState` object that contains the current location of the
 * user
 * @param {PositionOptions} [options] - PositionOptions
 * @returns The `useGeolocation` hook returns a `GeoLocationSensorState` object. This object contains
...
export const useGeolocation = (options?: PositionOptions): GeoLocationSensorState => {
    ...
    const onEventError = (error: IGeolocationPositionError) => setState(oldState => ({ ...oldState, loading: false, error }));
